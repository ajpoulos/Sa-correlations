...
for j in np.arange(npXi-1,-1,-1):
    ...
    for i,pT in enumerate(plotingTs):
        ...
        for k in range(nT):
            corrApprox[k] = correlationModel(T[k], xi2[j], pT, xi2[j])
...
xi1 = 0.01
indXi1 = np.where(xi==xi1)[0][0]
for j in np.arange(npXi):
    indXi2 = np.where(xi==xi2[j])[0][0]
    for i,pT in enumerate(plotingTs):
        indT1 = np.where(T==pT)[0][0]
        
        # Correlations from data
        corr = correlations[indXi1,indXi2,indT1,:]
        if i==0:
            label = r'$\xi_1$ = 1%, $\xi_2$ = '+str(int(xi2[j]*100))+r'%'
        ...
        corrApprox = np.zeros(nT)
        for k in range(nT):
            corrApprox[k] = correlationModel(T[k], xi1, pT, xi2[j])
